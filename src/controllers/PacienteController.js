...
const getAllPacientes = async (req, res) => {
  try {
    ...
    // Si es médico, obtener TODOS los pacientes (no solo los evaluados)
...
// Actualizar documentos de DNI
const updateDocumentosDNI = async (req, res) => {
  try {
    const { dniFrente, dniDorso } = req.body;
    ...
    if (!paciente.documentosIdentidad) {
      paciente.documentosIdentidad = {
        dniFrente: null,
        dniDorso: null
      };
    }
    ...
    // Actualizar solo los campos proporcionados
    if (dniFrente !== undefined) paciente.documentosIdentidad.dniFrente = dniFrente;
    if (dniDorso !== undefined) paciente.documentosIdentidad.dniDorso = dniDorso;

    paciente.markModified('documentosIdentidad');
    await paciente.save();

    res.status(200).json({
      success: true,
      data: paciente,
      message: 'Documentos de DNI actualizados exitosamente'
    });

  } catch (err) {
    console.error('Error en updateDocumentosDNI:', err);
    res.status(400).json({
      success: false,
      error: 'Error al actualizar documentos de DNI',
      details: err.message
    });
  }
};

// Agregar archivos REPROCANN
const agregarArchivoReprocann = async (req, res) => {
  try {
    const { nombre, url, tipo } = req.body;
    console.log('Body recibido:', req.body); // ← Agregar este log
    console.log('Params recibido:', req.params); // ← Y este
    const paciente = await Paciente.findById(req.params.id);
    ...
    if (!paciente.reprocann.archivos) {
      paciente.reprocann.archivos = [];
    }
    ...
    const nuevoArchivo = {
      nombre,
      url,
      tipo: tipo || 'aprobacion',
      fechaSubida: new Date()
    };

    paciente.reprocann.archivos.push(nuevoArchivo);
    paciente.markModified('reprocann.archivos');
    await paciente.save();

    res.status(201).json({
      success: true,
      data: paciente,
      message: 'Archivo REPROCANN agregado exitosamente'
    });

  } catch (err) {
    console.error('Error en agregarArchivoReprocann:', err);
    res.status(400).json({
      success: false,
      error: 'Error al agregar archivo REPROCANN',
      details: err.message
    });
  }
};

// Eliminar archivo REPROCANN
const eliminarArchivoReprocann = async (req, res) => {
  try {
    const { archivoId } = req.params;

    const paciente = await Paciente.findOne({
      'reprocann.archivos._id': archivoId
    });
    ...
    paciente.reprocann.archivos = paciente.reprocann.archivos.filter(
      archivo => !archivo._id.equals(archivoId)
    );
    ...
    res.status(200).json({
      success: true,
      data: paciente,
      message: 'Archivo REPROCANN eliminado exitosamente'
    });

  } catch (err) {
    console.error('Error en eliminarArchivoReprocann:', err);
    res.status(400).json({
      success: false,
      error: 'Error al eliminar archivo REPROCANN',
      details: err.message
    });
  }
};

// Agregar receta antigua
const agregarRecetaAntigua = async (req, res) => {
  try {
    const { nombre, url, fechaReceta, medico } = req.body;
    ...
    // Inicializar array si no existe
    if (!paciente.recetasAntiguas) {
      paciente.recetasAntiguas = [];
    }
    ...
    const nuevaReceta = {
      nombre,
      url,
      fechaReceta: fechaReceta ? new Date(fechaReceta) : null,
      medico: medico || '',
      fechaSubida: new Date()
    };

    paciente.recetasAntiguas.push(nuevaReceta);
    paciente.markModified('recetasAntiguas');
    await paciente.save();

    res.status(201).json({
      success: true,
      data: paciente,
      message: 'Receta antigua agregada exitosamente'
    });

  } catch (err) {
    console.error('Error en agregarRecetaAntigua:', err);
    res.status(400).json({
      success: false,
      error: 'Error al agregar receta antigua',
      details: err.message
    });
  }
};
...
const eliminarRecetaAntigua = async (req, res) => {
  try {
    const { recetaId } = req.params;

    const paciente = await Paciente.findOne({
      'recetasAntiguas._id': recetaId
    });
    ...
    paciente.recetasAntiguas = paciente.recetasAntiguas.filter(
      receta => !receta._id.equals(recetaId)
    );
    ...
    res.status(200).json({
      success: true,
      data: paciente,
      message: 'Receta antigua eliminada exitosamente'
    });

  } catch (err) {
    console.error('Error en eliminarRecetaAntigua:', err);
    res.status(400).json({
      success: false,
      error: 'Error al eliminar receta antigua',
      details: err.message
    });
  }
};
...
const getDocumentosPaciente = async (req, res) => {
  try {
    const paciente = await Paciente.findById(req.params.id)
      .select('documentosIdentidad reprocann.archivos recetasAntiguas fullName');
    ...
    res.status(200).json({
      success: true,
      data: {
        documentosIdentidad: paciente.documentosIdentidad || {},
        archivosReprocann: paciente.reprocann?.archivos || [],
        recetasAntiguas: paciente.recetasAntiguas || []
      },
      paciente: paciente.fullName
    });

  } catch (err) {
    console.error('Error en getDocumentosPaciente:', err);
    res.status(500).json({
      success: false,
